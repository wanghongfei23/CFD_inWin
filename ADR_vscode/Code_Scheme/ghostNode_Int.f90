...
Subroutine TCNS_A_O5_ai(nVar, var_In, var_Out)
    ...
    ! 计算eta数组
    eta(1) = (abs(2.0d0 * delta_q(1) * delta_q(2)) + epsilon_A) / &
             (delta_q(1)**2 + delta_q(2)**2 + epsilon_A)
    
    eta(2) = (abs(2.0d0 * delta_q(2) * delta_q(3)) + epsilon_A) / &
             (delta_q(2)**2 + delta_q(3)**2 + epsilon_A)
    
    eta(3) = (abs(2.0d0 * delta_q(3) * delta_q(4)) + epsilon_A) / &
             (delta_q(3)**2 + delta_q(4)**2 + epsilon_A)
...
! 王鸿飞
!! ============== TCNS-ASF103 ============== !!
Subroutine TCNS_ASF103_O5_ai(nVar, var_In, var_Out)
    Use Parameters
    Implicit none
    ...
    ! 计算C_T
    C_T = 1.0d0 / (4068.43d0 * min_val * min_val * min_val - 208.997d0 * min_val * min_val - 11.9427d0 * min_val+  6.81529d0)
...
End Subroutine TCNS_ASF103_O5_ai


!! ============== TCNS-ASF102_var_01 ============== !!
Subroutine TCNS_ASF102_var_01_O5_ai(nVar, var_In, var_Out)
    Use Parameters
    Implicit none
    ...
    ! 计算C_T
    C_T = 0.157d0 / (6.195795d0 * xx * xx - 2.840437d0 * xx + 1.231848d0)
...
End Subroutine TCNS_ASF102_var_01_O5_ai

!! ============== TCNS-ASF103_var_01 ============== !!
Subroutine TCNS_ASF103_var_01_O5_ai(nVar, var_In, var_Out)
    Use Parameters
    Implicit none
    ...
    ! 计算C_T
    C_T = 1.0d0 / (1422.98d0 * min_val * min_val * min_val + 172.859d0 * min_val * min_val - 26.2072d0 * min_val + 6.8629d0)
...
End Subroutine TCNS_ASF103_var_01_O5_ai




!! ============== TCNS-ASF203 ============== !!
Subroutine TCNS_ASF203_O5_ai(nVar, var_In, var_Out)
    Use Parameters
    Implicit none
    ...
    ! 计算min_val
    min_val = min(1.0d0, eta_min/0.24)
    
    m = 1-min_val

    ! 计算C_T
    C_T = -0.452662*m*m*m + 0.590811*m*m - 0.012189*m + 0.022010
...
End Subroutine TCNS_ASF203_O5_ai
